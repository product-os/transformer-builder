...
import {BundledTransformerContract, ProcessedContractDefinition, TransformerContract} from "../src/types";
...
describe('convertContract',  function( ) {
    const manifest = {
        input: {
            contract: yaml.load(fs.readFileSync('input/artifact/balena.yml', 'utf8')) as ProcessedContractDefinition,
        }
    };
    const contracts: ContractDefinition[] = yaml.loadAll(fs.readFileSync('input/artifact/contracts.yml', 'utf8')) as ContractDefinition[];
    ...
    it('should return a valid transformer contract', async function ( ) {
        expect(await createContract(transformerContractWithRef, contracts,
    ...
    it('should embed the input schema filter as "inputFilter"', async function () {
        expect(await createContract(transformerContractWithSchema, contracts,
            manifest.input.contract.version,
    ...
    it('should throw an error if both "input.filter" & "input.$ref" are undefined',  async function () {
        await expect(async () => {await createContract(transformerContractWithEmptyInput, contracts,
            manifest.input.contract.version,
        )}).rejects.toThrowError('ERROR: Could not create transformer contract. Both $ref & filter are undefined.')
    });
    ...
    it('should throw an error if "input.$ref" is undefined & "input.filter" is not a valid JSON schema.',  async function () {
        await expect(async () => {await createContract(transformerContractWithInvalidSchema, contracts,
            manifest.input.contract.version,
        )}).rejects.toThrowError('ERROR: Could not create transformer contract. "input.filter" is not a valid JSON schema.')
    });
})